...
// On Render: credentials come from FIREBASE_SERVICE_ACCOUNT env var (JSON string)
// Locally: set the same env var or use a local service account key file
...
const entries = {
  async all() {
    ...
    return docs.map(e => {
      ...
      return { ...e, taskName: t ? t.name : 'Unknown', projectName: t?.projectId ? (projectMap[t.projectId] || null) : null };
    });
  },
  async get(id) {
